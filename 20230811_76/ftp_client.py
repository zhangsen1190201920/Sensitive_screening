...
class FtpClient(threading.Thread):
    ...
    def __init__(self, host, port, username, password, remote_dir, local_dir, interval=3):
        super().__init__()
        self.ftp = FTP()
        self.host = host
        self.port=port
        ...
        logging.info(f"远程 remote_dir：{remote_dir}")
    ...
    def update_date(self):
        ...
        """
        today = get_today_str()
        if today not in self.remote_dir:
            self.remote_dir = re.sub(r'\d{8}', today, self.remote_dir)
            self.local_dir = re.sub(r'\d{8}', today, self.local_dir)# 使用正则表达式re.sub()函数将self.remote_dir字符串中的八位数字替换为today，并将结果存储回self.remote_dir中。
            ...
            with open(error_file, "r") as f:
                for line in f:
                    if remote_file in line:
                        return True
        return False
    def process_remote_files(self):
        """
        处理远程服务器上的日志文件，将它们下载到本地并进行处理
        """
        remote_files = [x for x in self.ftp.nlst() if os.path.splitext(x)[-1] == ".log"]
        for remote_file in remote_files:
            if not self.is_log_failed(remote_file): # 如果log也不在本地的error的日志文件中错误中
                self.process_remote_log(remote_file)
        ...
        if not self.is_log_downloaded(remote_file) and not self.is_tmp_file_downloaded(remote_file):
            ...
            try:
                ...
                if file_type not in self.file_types:
                    ...
                    shutil.rmtree(local_tmp_log_path)
